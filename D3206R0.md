...
Awaitables have a very similar way to convey this information by returning `true` or `false` for `await_ready()` but without this proposal senders in `std::execution` are missing a tool to do so.
...
The customization point object `execution::get_completion_behaviour(s, env)` is proposed.

Similarily to `get_completion_signatures(s, env)` a member method on the sender is being proposed, i. e.
...
struct example_sender_adaptor {
  ...
  constexpr auto get_completion_behaviour(this Self&& self, Env&& env) {
...
And if the completion behaviour is statically known and does not depend on the environment a short
...
The main difference of this proposal to the implementation at libunifex is, that the query in this proposal acts on additionally on the environment instead of only a sender.
...
- `get_completion_behaviour(s, env)`: 
  - `get_completion_behaviour(sender, env)`, if `get_completion_behaviour(sender, env) >= synchronous`
  -  a dynamic value, otherwise. The operation of `split(sender)` completes inline if the input operation of split has completed before starting a copy of split.