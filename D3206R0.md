...
If the completion behaviour is statically known and does not depend on the environment a shorter typedef can be used to define the completion behaviour.
...
Another alternative is to consider a boolean predicate, such as `completes_synchronously(s, env)`, instead.
Note, that `inline_completion` implies `synchronous` completion.
In practise, standardized senders that are synchronous will complete inline but it is the synchronous completion behaviour that enables most optimizations.
The current proposal can distinguish between both completion types.
